...
class test_prm_v4(PRM):
    def __init__(
        ...
        # self.bad_token = '-'
        self.step_tag = ' ки'
        self.model_id = "Daewon0808/v4_llama_lora"
        ...
        # print(self.tokenizer.encode(f"{self.step_tag}"))
        # input("------------")   
        self.step_tag_id = self.tokenizer.encode(f"{self.step_tag}")[-1] # 17165
        # print(self.tokenizer.decode(self.tokenizer.encode(f"{self.step_tag}"), skip_special_tokens=False))
        # print("*****")
        ##
