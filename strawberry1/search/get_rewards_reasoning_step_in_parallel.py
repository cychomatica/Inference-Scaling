...
def main():
    ...
    if not all_file_exists:
        ...
        elif test_prm == "rlhflow-ds_qwen_fulltune":
            from rlhflow_ds_qwen_fulltune import test_prm_dual
            prm = test_prm_dual(
                aggregation="full", 
            )
        ...
        elif test_prm == "mmlu_math_noaugs_llama_lora":
            from v5_llama_lora import test_prm_dual
            prm = test_prm_dual(
                aggregation="full", 
                model_id="Daewon0808/mmlu_math_noaugs_llama_lora"
            )
        elif test_prm == "mmlu_small_noaugs_llama_lora":
            from v5_llama_lora import test_prm_dual
            prm = test_prm_dual(
                aggregation="full", 
                model_id="Daewon0808/mmlu_small_noaugs_llama_lora"
            )
        else:
    ...
    for example_file_path in example_file_path_list:
        ...
        if args.sample_10:
            random.seed(42)
            data_all = random.sample(data_all, 10)
        ##
