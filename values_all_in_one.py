...
if __name__ == '__main__':
    ...
    models_to_eval = {}
    for model in ['math_psa', 'math_shepherd', 'qwen2.5_math_7b_prm800k', 'rlhflow_deepseek', 'prm800k_llama_fulltune', 'prm800k_qwen_fulltune']:
    ...
    for category in categories:
        ...
        for method in ['last', 'mean', 'min']:
            ...
            for model in models_to_eval:
                ...
                # best_of_n_y_list[model] = best_of_n_y[-1]
                # weighted_majority_voting_y_list[model] = weighted_majority_voting_y[-1]
                best_of_n_y_list.append(best_of_n_y[idx_N])
                weighted_majority_voting_y_list.append(weighted_majority_voting_y[idx_N])
            
            majority_voting.append(majority_y[idx_N])
            values[category][method] = {}
            values[category][method]['best_of_n'] = [round(num, 2) for num in best_of_n_y_list + majority_voting]
            values[category][method]['weighted_majority_voting'] = [round(num, 2) for num in weighted_majority_voting_y_list + majority_voting]
            # values[category][method] = {'majority_voting': majority_voting, 'best_of_n': best_of_n_y_list, 'weighted_majority_voting': weighted_majority_voting_y_list}

    improvement = {}
    take = {}
    for category in categories:
        raw = values[category]['min']['weighted_majority_voting']
        improvement[category] = [round((x - raw[-1]) / raw[-1] * 100, 2) for x in raw[:-1]]
        for d in range(len(improvement[category])):
            if improvement[category][d] >= 0:
                improvement[category][d] = '(+{:.2f}\%)'.format(improvement[category][d])
            else:
                improvement[category][d] = '({:.2f}\%)'.format(improvement[category][d])
        improvement[category].append('')

        take[category] = ''.join(['{:.2f}{} & '.format(x, d) for (x, d)in zip(raw, improvement[category])])[:-2] + '\\\\'
        print(category , '\n', take[category])
